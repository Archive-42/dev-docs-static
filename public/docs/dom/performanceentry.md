...
The `PerformanceEntry` object encapsulates a single performance metric that is part of the _performance timeline_. A _performance entry_ can be directly created by making a performance _[`mark`](performancemark)_ or _[`measure`](performancemeasure)_ (for example by calling the [`mark()`](performance/mark) method) at an explicit point in an application. Performance entries are also created in indirect ways such as loading a resource (such as an image).
