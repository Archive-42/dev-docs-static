...
The `Window.location` read-only property returns a [`Location`](../location) object with information about the current location of the document.

Though `Window.location` is a *read-only* `Location` object, you can also assign a [`DOMString`](../domstring) to it. This means that you can work with `location` as if it were a string in most cases: `location = 'http://www.example.com'` is a synonym of `location.href = 'http://www.example.com'`.

See [`Location`](../location) for all available properties.
...
Consider the following example, which will reload the page by using the [`replace()`](../location/replace) method to insert the value of `location.pathname` into the hash:
...
<span class="small">The definition of 'Window.location' in that specification.</span></a></td><td><span class="spec-living">Living Standard</span></td><td>No change from <a href="https://www.w3.org/TR/html52/">HTML5</a>.</td></tr><tr class="even"><td><a href="https://www.w3.org/TR/html52/browsers.html#the-location-interface">HTML5<br />
<span class="small">The definition of 'Window.location' in that specification.</span></a></td><td><span class="spec-rec">Recommendation</span></td><td>Initial definition.</td></tr></tbody></table>
...
-   A similar information, but attached to the document, [`Document.location`](../document/location).
-   [Manipulating the browser history](../history_api)
-   `hashchange`

<a href="https://developer.mozilla.org/en-US/docs/Web/API/Window/location" class="_attribution-link">https://developer.mozilla.org/en-US/docs/Web/API/Window/location</a>
