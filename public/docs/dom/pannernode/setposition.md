...
The `setPosition()` method of the [`PannerNode`](../pannernode) Interface defines the position of the audio source relative to the listener (represented by an [`AudioListener`](../audiolistener) object stored in the [`BaseAudioContext.listener`](../baseaudiocontext/listener) attribute.) The three parameters `x`, `y` and `z` are unitless and describe the source's position in 3D space using the right-hand Cartesian coordinate system.
